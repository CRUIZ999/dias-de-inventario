// app.js
// Panel ligero para leer un CSV (delimitado por comas) y mostrarlo en un tablero

let rawData = []; // todos los registros
let filteredData = []; // registros luego de filtros actuales
...
function handleFileUpload(ev) {
  ...
  reader.onload = (e) => {
    ...
    document.getElementById(
      "fileStatus"
    ).textContent = `Archivo cargado: ${file.name} · ${rawData.length} filas`;
...
// 2) Parse simple de CSV (sin comillas anidadas)
...
function parseNumber(v) {
  if (!v) return 0;
  // Por si vienen comas de miles, las quitamos
...
function aplicarFiltrosYRender(resetSort = false) {
  ...
  // Búsqueda por código/clave/desc_prod
  if (state.search) {
  ...
  // Filtro cobertura (Mes)
  if (state.filtroCobertura !== "all") {
    data = data.filter((row) => {
      const c = row.cobertura_mes;
      if (state.filtroCobertura === "critico") {
        return c >= 0 && c <= 1;
      }
      if (state.filtroCobertura === "medio") {
        return c > 1 && c <= 3;
      }
      if (state.filtroCobertura === "alto") {
        return c > 3;
      }
      return true;
    });
  ...
  // Si cambiamos mucho el filtro, podemos resetear orden
  if (resetSort) {
    sortConfig.key = null;
...
function renderKPIs(data) {
  ...
  if (!data.length) {
    kpiProductos.textContent = "0 / " + total;
    kpiInventario.textContent = "0";
    kpiPromedioVenta.textContent = "0";
    kpiCobertura.textContent = "0";
    return;
  }
...
function renderTable(data) {
  ...
  data.forEach((row) => {
    ...
    // Cobertura Días (30) con pequeño heat map
    ...
    if (d <= 30) {
      tdCobDias.classList.add("heat-low");
    } else if (d > 30 && d <= 90) {
      tdCobDias.classList.add("heat-mid");
    } else if (d > 90) {
      tdCobDias.classList.add("heat-high");
    }
...
function toggleClasifChip(value) {
  if (state.clasifActivas.has(value)) {
    state.clasifActivas.delete(value);
  } else {
    state.clasifActivas.add(value);
  }
  sincronizarChipsUI();
  aplicarFiltrosYRender();
}
